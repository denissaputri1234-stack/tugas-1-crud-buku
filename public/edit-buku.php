...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($_FILES["cover"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["cover"]["tmp_name"], $target)) {
            $cover_baru = "uploads/" . $nama_file;

            // Hapus cover lama
            if ($cover_lama && file_exists(__DIR__ . "/" . $cover_lama)) {
                unlink(__DIR__ . "/" . $cover_lama);
            }
        } else {
            $errors[] = "Gagal upload cover baru!";
        }
    }

    if (empty($errors)) {
        $sql = "UPDATE buku SET judul=:judul, penulis=:penulis, penerbit=:penerbit,
                tahun_terbit=:tahun_terbit, kategori=:kategori, cover_path=:cover_path
                WHERE id=:id";
...
<div class="container">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="kategori">
            ...
            <option <?= $buku['kategori']=="Bisnis"?"selected":"" ?>>Bisnis</option>
            <option <?= $buku['kategori']=="Lainnya"?"selected":"" ?>>Lainnya</option>
        </select>

        <label>Cover Saat Ini</label>
        <?php if ($buku['cover_path']): ?>
            <img src="<?= $buku['cover_path'] ?>">
        <?php endif; ?>
